    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="text-center py-8 sm:py-12 bg-white rounded-2xl border border-gray-200">
        <div className="flex items-center justify-center mb-4 sm:mb-6">
          <div className="w-12 h-12 sm:w-16 sm:h-16 bg-[#003399] rounded-2xl flex items-center justify-center shadow-sm">
            <GraduationCap className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
          </div>
        </div>
        <h1 className="text-2xl sm:text-4xl md:text-5xl font-bold text-[#003399] mb-3 sm:mb-4">Campus-AI Chat</h1>
      ...
      <Card className="bg-white border-gray-200">
        ...
        <CardContent>
          <div className="h-[300px] sm:h-[420px] overflow-y-auto space-y-4 pr-1">
            {messages.map((m, idx) => (
              <div key={idx} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`${m.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-900'} px-3 sm:px-4 py-2 sm:py-3 rounded-2xl max-w-[85%] sm:max-w-[80%] shadow-sm`}>
                  <div className="text-xs sm:text-sm whitespace-pre-wrap leading-relaxed">{m.content}</div>
                  <div className={`${m.role === 'user' ? 'text-blue-100' : 'text-gray-500'} text-[9px] sm:text-[10px] mt-1 text-right`}>{m.time}</div>
                </div>
              </div>
            ))}
            <div ref={endRef} />
          ...
          <div className="mt-4 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            <input
              ...
              className="flex-1 border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-200 text-sm sm:text-base"
              disabled={isLoading}
            />
            <Button 
              onClick={handleSend} 
              disabled={isLoading || !input.trim()}
              className="bg-[#E60023] hover:bg-[#cc001f] text-white disabled:opacity-50 text-sm sm:text-base"
            >
              {isLoading ? (
